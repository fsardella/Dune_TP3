...
required(num_required),game_name(name), yamlMapPath(yamlMapPath) {
    ...
    std::vector<std::vector<int>> bases = node["constructions"].as<std::vector<
                                                std::vector<int>>>();
    for (auto& row : bases) {
        this->basesCoordinates.push_back(coor_t(row[0] * CHUNKSIZE, row[1] * CHUNKSIZE));
    }
}
...
Game::Game(Game&& other) : required(other.required),
                           ...
                           basesCoordinates(std::move(other.basesCoordinates)),
                           decidedWinner(other.decidedWinner) {}
...
Game& Game::operator=(Game&& other) {
    ...
    this->basesCoordinates = std::move(other.basesCoordinates);
    this->required = other.required;
    this->decidedWinner = other.decidedWinner;
    other.required = 0;
    return *this;
}

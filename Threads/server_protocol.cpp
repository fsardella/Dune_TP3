...
tener la informacion para listar las partidas actuales.
*/

void ProtocolServer::send_msg_list(Game game) {
	uint8_t num_participants_convert = convert_to_uint8(game.get_participants());
	socket.sendall(&num_participants_convert, 1);

	uint8_t num_required_convert = convert_to_uint8(game.get_requiered());
